...
@WebAppConfiguration
public class WeixiangyouWSImplTest  extends AbstractJUnit4SpringContextTests {

    public static class SimpleServer extends AbstractBusTestServerBase {
        protected void run() {
            Object implementor = new WeixiangyouWSImpl();
            String address = "http://localhost:" + 8080 + "/soap/WeixiangyouWS";//设置服务端的接口地址
            Endpoint.publish(address, implementor);//发布接口
        }
    /*    public static void main(String[] args) {
            try {
                WeixiangyouWSImplTest.SimpleServer s = new WeixiangyouWSImplTest.SimpleServer();
                s.start();
            } catch (Exception ex) {
                ex.printStackTrace();
                System.exit(-1);
    ...
    public void sendMsg() throws Exception {
        ...
        Client client = dynamicClient.createClient("http://localhost:8080/soap/WeixiangyouWS?wsdl");//创建一个客户端，设置好请求地址
    ...
    @Before
    public void init() {
        launchServer(WeixiangyouWSImplTest.SimpleServer.class, true);
    }
